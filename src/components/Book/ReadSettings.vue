    ...
    <div class="setting-list">
      <ul>
        <li class="theme-list">
          ...
          ><em v-if="index < 6" class="iconfont">&#58980;</em><em v-else class="iconfont moon-icon">{{ moonIcon }}</em></span>
        </li>
        <li class="font-list">
          <i>正文字体</i>
          <span
        ...
        <!--              >确定</el-button>-->
        <!--            </div>-->
        <!--            <span slot="reference" type="text" class="font-item">保存</span>-->
        <!--          </el-popover>-->
        <!--        </li>-->
        <li class="font-size">
          <i>字体大小</i>
          <div class="resize">
            <span
              class="less"
              @click="lessFontSize"
            ><em class="iconfont">-</em></span><b /> <span class="lang">{{ fontSize }}</span><b />
            <span
              class="more"
              @click="moreFontSize"
        ...
        <li v-if="!$store.state.miniInterface" class="read-width">
          <i>页面宽度</i>
          <div class="resize">
            <span
              class="less"
              @click="lessReadWidth"
            ><em class="iconfont">-</em></span><b /> <span class="lang">{{ readWidth }}</span><b />
            <span
              class="more"
              @click="moreReadWidth"
            ><em class="iconfont">+</em></span>
...
export default {
  ...
  data() {
    return {
      ...
      fonts: ['雅黑', '宋体', '楷书'],
      customFontName: this.$store.state.bookConfig.config.customFontName,
      customFontSavePopVisible: false
    }
  },
  computed: {
    ...
    selectedTheme() {
      return Number(this.$store.state.bookConfig.config.theme)
    },
    selectedFont() {
      return this.$store.state.bookConfig.config.font
    },
    fontSize() {
      return this.$store.state.bookConfig.config.fontSize
    },
    readWidth() {
      return this.$store.state.bookConfig.config.readWidth
    },
    infiniteLoading() {
  ...
  mounted() {
    // 初始化设置项目
    const config = this.$store.state.bookConfig.config
    this.theme = config.theme
    if (Number(this.theme) === 6) {
      this.moonIcon = ''
    } else {
      this.moonIcon = ''
    }
  ...
  methods: {
    ...
    saveConfig(config) {
      this.$store.commit('bookConfig/setConfig', config)
...
.settings-wrapper {
  ...
  background: #ede7da url('../../assets/themes/popup_1.png') repeat;
