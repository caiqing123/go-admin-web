<template>
  <div class="session-panel-warp iChat-session-warp">
    <div class="iChat-session-header">
      <span class="iconfont icon-left el-icon-arrow-left" @click="back" />
      <div class="session-info-warp">
        <img src="@/assets/chat/group-icon.png" alt="" class="session-avatar">
    ...
    <div class="iChat-session-footer">
      <div class="iChat-message-form" :class="{'focus-form':text!==''||isFocus}">
        <div class="iChat-send-warp">
          <button v-show="text!==''||isFocus" class="iChat-send-btn" @click="sendText(text)">发送</button>
          <span v-show="text===''&&!isFocus" class="iconfont el-icon-circle-plus-outline" @click.stop="toggleTool" />
        </div>
        <div class="iChat-expression-btn" @click.stop="toggleExpression">
          <van-icon class="iconfont" name="smile-o" />
        </div>
        <div class="iChat-input-warp">
          <input v-model="text" type="text" class="iChat-message-input" @focus="isFocus=true" @keypress.enter="sendText(text)" @focusout="isFocus=false">
        </div>
      </div>
      <div v-show="isShowExpression" class="iChat-expression-panel scroll">
        <ul class="expression-list">
          <li v-for="item in expressions" :key="item.title" @click.stop="pickerExpression(item)">
            <img :src="require('@/assets/chat/emoji'+item.url)" alt="">
          </li>
        </ul>
      </div>
      <div v-show="isShowTool" class="iChat-tool-panel scroll">
...
export default {
  ...
  methods: {
    ...
    sendMessage(html, type) {
      this.$emit('sendMessage', html, type)
    },
    toggleTool() {
      const _this = this
      function hideTool() {
