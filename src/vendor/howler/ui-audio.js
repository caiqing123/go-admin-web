...
class UiAudio extends HTMLElement {
  ...
  set volume(value) {
    ...
    if (value || value === 0) {
      const range = this.element && this.element.volumeRange
      range.value = value
  ...
  connectedCallback() {
    ...
    ele.loop.addEventListener('change', (event) => {
      ...
      const optionSelected = target.selectedOptions[0] || target.options[0]
      if (!optionSelected) {
        return
      }
